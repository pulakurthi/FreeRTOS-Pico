...
Bare-minimum FreeRTOS project for the **Pico 2 (RP2350, Cortex-M33)** that blinks the onboard LED.

## Prerequisites

- Raspberry Pi Pico SDK **2.2.0**
- FreeRTOS Kernel (latest `main` branch)
...
| Macro | Value | Meaning |
|-------|-------|---------|
| `configCPU_CLOCK_HZ` | 150 000 000 | RP2350 default clock speed (150 MHz). |
...
| `configMAX_PRIORITIES` | 5 | Priority levels 0 (idle) through 4. |
...
| `configTOTAL_HEAP_SIZE` | 16 KB | Total RAM available to the FreeRTOS heap allocator. |
| `configKERNEL_PROVIDED_STATIC_MEMORY` | 1 | The kernel provides RAM for the idle task and timer task when static allocation is enabled. |

#### Software Timers

Enabled (`configUSE_TIMERS = 1`). The timer service task runs at the highest priority (`configMAX_PRIORITIES - 1`) with a queue depth of 10.

#### Hook Functions

| Macro | Value | Meaning |
|-------|-------|---------|
| `configCHECK_FOR_STACK_OVERFLOW` | 2 | Enables both method-1 (high water mark) and method-2 (pattern check) stack overflow detection. Calls `vApplicationStackOverflowHook()` on overflow. |
| `configUSE_IDLE_HOOK` / `configUSE_TICK_HOOK` | 0 | No idle or tick hooks — avoids the need to define empty callback stubs. |

#### Debug Assert

`configASSERT(x)` disables interrupts and halts in an infinite loop if the assertion fails. Helps catch configuration errors during development.

#### Cortex-M33 / RP2350 Specifics

| Macro | Value | Meaning |
|-------|-------|---------|
| `configKERNEL_INTERRUPT_PRIORITY` | 0 | Kernel runs at the lowest urgency interrupt level. |
